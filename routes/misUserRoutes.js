...
misUserRouter.post("/login", (req, res) => {
  ...
  connection.query(queryToFetchUser, [email_ID], (err, result) => {
    if (err) {
      ...
      connection.query(queryToFetchPassword, [email_ID], (err, result_pwd) => {
        ...
        if (result_pwd) {
          ...
          hashPassword(PASSWORD).then(() => {
            ...
            verifyPassword(PASSWORD, selected_user_details.PASSWORD).then((resp) => {
              if (resp) {
                ...
                connection.query(queryToDetectDuplicateSession,[selected_user_details.email_ID], (err, res_dup_sessions)=>{
                  if(err){
                    // res.status(500).json({ error: "Internal server error" });
                    // return;
                    console.log(err);
                    ...
                    connection.query(queryToDeleteDupSessions, (err, del_result) =>{
...
misUserRouter.post("/logout", authMisUser, (req, res) => {
  ...
  connection.query(queryToDeleteSession, [SESSION_ID], (err) => {
    if (err) {
...
misUserRouter.patch("/updatePassword",authMisUser, async (req, res)=>{
  ...
  connection.query(queryToFetchUser, [email_ID], async (err, result) => {
    if (err) {
      console.error("Error checking credentials:", err.message);
      res.status(500).json({ error: "Internal server error" });
      return;
    ...
    if (result) {
      ...
      connection.query(updateMISuser,[newPassword, selected_user_details.email_ID],(err, updResult)=>{
        if (err) {
          console.error(err.message);
...
misUserRouter.post("/purchaseGames", authMisUser, async(req, res)=>{
  ...
  try {
    ...
    let { client_id, games_purchased, Pack_valid_till } = req.body;
    const purchase_date = formatDate(new Date());

    let purchases = games_purchased;

    // Parse purchases string into an array if necessary
    if (typeof purchases === 'string') {
        try {
          const cleanInput = purchases.replace(/^"|"$/g, ''); 
            purchases = cleanInput.split(',');
        } catch (err) {
...
connection.beginTransaction((err) => {
  if (err) {
    ...
    return res.status(500).send("Transaction start failed: " + err.message);
  }

  // Track errors manually
  let hasError = false;

  purchases.forEach(( game_id , index) => {
    const values = [
      client_id,
      game_id,
      purchase_date,
    ...
    connection.query(querytoPurchaseGames, values, (err) => {
      if (err) {
        hasError = true;
        console.error("Error inserting record:", err);
        return connection.rollback(() => {
          res.status(500).send("Error adding records: " + err.message);
        });
      }

      // When it's the last item and no errors occurred, commit
      if (index === purchases.length - 1 && !hasError) {
        connection.commit((err) => {
          if (err) {
            return connection.rollback(() => {
              console.error("Commit failed:", err);
              res.status(500).send("Transaction commit failed: " + err.message);
            });
          }

          res.json({ message: "Games purchase successful!" });
        });
      }
    });
  });
...
misUserRouter.get("/clientpurchasedGames", authMisUser, async (req, res)=>{
  ...
  try {
    ...
    connection.query(queryToFetchClientPurchasedGames, [client_id], (err, result) => {
        if (err) {
